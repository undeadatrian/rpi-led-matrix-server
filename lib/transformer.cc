...
/***********************************/
/* Rectangle Transformer Canvas */
/***********************************/
class RectangleTransformer::TransformCanvas : public Canvas {
public:
  TransformCanvas() : delegatee_(NULL) {}
...
void RectangleTransformer::TransformCanvas::SetDelegatee(Canvas* delegatee) {
  // Our assumptions of the underlying geometry:
...
void RectangleTransformer::TransformCanvas::Clear() {
  delegatee_->Clear();
}

void RectangleTransformer::TransformCanvas::Fill(uint8_t red, uint8_t green, uint8_t blue) {
  delegatee_->Fill(red, green, blue);
}

int RectangleTransformer::TransformCanvas::width() const {
  return 128;
}

int RectangleTransformer::TransformCanvas::height() const {
  return 32;
}

void RectangleTransformer::TransformCanvas::SetPixel(int x, int y, uint8_t red, uint8_t green, uint8_t blue) {
  if (x < 0 || x >= width() || y < 0 || y >= height()) return;
  // We have up to column 64 one direction, then folding around. Lets map
  if (y > 15) {
    x = 255 - x;
...
/****************************/
/* Rectangle Transformer Transformer */
/****************************/
...
Canvas *RectangleTransformer::Transform(Canvas *output) {
  assert(output != NULL);
